...
namespace Entidades
{
    public class Electrica : Guitarra,ICircuito,IProduccion
    {
        ...
        public static event CargarGuitarra ElectricaAgregada;
        ...
        {
            if (this.Encordado is ECuerdas.DaddarioElectrica && ValidarControlStock(StockElementos.encordadosDaddarioElectrica, 1))
            {
                StockElementos.encordadosDaddarioElectrica -= 1;
                return true;
            }

            if (this.Encordado is ECuerdas.ErnieBallElectrica && ValidarControlStock(StockElementos.encordadosErnieBallElectrica, 1))
            {
        ...
        public bool DisminuirStock()
        {
            ...
            if ((this.Encordado is ECuerdas.ErnieBallElectrica && ValidarControlStock(StockElementos.encordadosErnieBallElectrica, 1)) || (this.Encordado is ECuerdas.DaddarioElectrica && ValidarControlStock(StockElementos.encordadosDaddarioElectrica, 1)))
            {
                encordadoValidado = true;
            }

            if (micValidado && clavijeroValidado && encordadoValidado)
            {
            ...
            //Si se cuenta con todos los elementos y el retorno vale TRUE, realizo la invocación
            if (Electrica.ElectricaAgregada != null && retorno == true)
            {
                Electrica.ElectricaAgregada.Invoke();
            }
